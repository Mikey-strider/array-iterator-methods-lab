...
// 1. Filter the array of inventors into a new array containing 
// only the inventors born in the 1500's.

// - You have an array of inventors, each with a birth year 
// listed by the property 'year'.
// - Use the Array.prototype.filter() method to create a new array.
// - The new array, 'veryOldInventors', should only include 
// inventors born between the years 1500 and 1599.


let veryOldInventors = []

// Enter your solution code here:
veryOldInventors = inventors.filter((inventor) => inventor.year < 1600 && inventor.year >= 1500)
...
// - For each inventor, 
// return an object in the format { first: "First Name", last: "Last Name" }.
...
// 4. Use Array.prototype.find() to identify the inventor
//  with the first name 'Ada' from an array of inventor objects

// - You have an array of objects, each representing an 
// inventor with various properties including their first name.

// -  Utilize the Array.prototype.find() method to locate
//  the object for the inventor named 'Ada'.
...
firstLast = people.map((person) => {
  const names = person.split(' ').reverse();
  return names.join(' ')
})
...
// 6. Determine if there is at least one person in the 
// devs array who is 18 years old or older.

// - You have an array of people with their respective ages.
// - Use the Array.prototype.some() method to check if any person 
// in the array is 18 years old or older.
...
isAdultPresent = devs.some((dev) => {
  const overAge = todaysDate - dev.year;
  console.log(overAge > 18);
  return overAge > 18;
})
...
// 7. Use Array.prototype.every() to determine if every person in the 
// devs array is 19 years old or older.

// - You have an array of individuals, 
// each with their year of birth represented by the 'year' property.
// - Use the Array.prototype.every() 
// method to verify if every individual in the array is at least 19 years old.
...
// 8. Use Array.prototype.find() to identify and retrieve the 
// comment object with a specific ID 823423 from an array of comment objects.
...
// - Each object in the array includes properties: 'first', 'last', 'year' (birth year), 
// and 'passed' (year of death).
...
// - The resulting object should have keys as the travel 
// methods ('car', 'truck', 'bike', etc.) and values as their respective counts.
// - Store this object in the variable 'travelMethodCounts'.

// Hints:
// - Inside the reduce function, check if the travel method already 
// exists as a key in your accumulator object. If it does, increment its count. If not, add it with a count of 1.
// - Since you want to return an object, be sure to pass an empty
...
travelMethodCounts = travelMethods.reduce((acc, travelMethod) => {
  console.log(acc, '<- accumelator', travelMethod, '<-- type of travel');
  if(acc[travelMethod]){
    acc[travelMethod] + 1;
  } else {
    acc[travelMethod] = 1
